...
pub const Tz = struct {
    ...
    fn parseBlock(allocator: std.mem.Allocator, reader: *std.Io.Reader, header: Header, legacy: bool) !Tz {
        ...
        if (!legacy) {
            ...
            const footer_mem = reader.takeSentinel('\n') catch |err| switch (err) {
                error.StreamTooLong => return error.ReadBufferTooSmall,
                else => return err,
            };
            if (footer_mem.len > 128) return error.OverlargeFooter; // Read more than 128 bytes, much larger than any reasonable POSIX TZ string
            if (footer_mem.len != 0) {
...
test "slim" {
    ...
    var tz = try Tz.parse(std.testing.allocator, &in_stream);
    defer tz.deinit();
